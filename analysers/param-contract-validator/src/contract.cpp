...
/**
 * Copy contract and specialize it wrt. argument
 */
Contract::Contract(const Contract& c, const std::string& arg)
{
  parse(c.original_line, arg);
}
 ...
 * Simply: s/\$[1-9]/$to/g
 */
void substitute_argument(std::string& str, const std::string& to)
{
  // TODO escape '$' in argument value
  assert(to.find("$") == std::string::npos);
  do {
...
/**
 * Parse and build a contract from one-line string representation.
 * @param line Contract definition
 * @param arg Argument specialization
 */
void Contract::parse(const std::string& line, const std::string& arg)
{
#if VERBOSITY_LEVEL >= 10
  CONSOLE("Parsing contract line: " + line + " with arg='" + arg + "'\n");
#endif
  original_line = line;
  argument = arg;
  ...
  std::string first = mo[1].str();
  std::string second = mo[2].str();
  // substitute '$' with arg
  if (!arg.empty())
  {
    substitute_argument(first, arg);
    substitute_argument(second, arg);
  }

  // Process the target first
  Target* target = new Target(g_currTargetType++);

  // Create a FA which represents the target
  target->fa = this->construct(first);

  // If more spoilers can violate a target, they are separated by a comma
//  Tokenizer spoilers(second, boost::char_separator< char >(","));
  Tokenizer spoiler_tokens(second, boost::char_separator< char >(","));

  for (const std::string& token : spoiler_tokens)
  { // Process all spoilers which may violate the target
    Spoiler* spoiler = new Spoiler(g_currSpoilerType++);

    // Create a FA which represents the spoiler
    spoiler->fa = this->construct(boost::trim_copy(token));
...
{
  CONSOLE("::constructing contract " + regex + "\n");
  // Helper variables
  ...
  { // Transform the regular expression into a finite automaton
    ...
    { // Process the current token
      ...
      default: // Name of a method
        ...
        add_parametrized(boost::trim_copy(*it));
        break;
    }
  }
...
ContractList Contracts::add_parameter(const std::string &arg)
{
  ContractList newcntrs;

  // search for the argument, return if already exists
  for (Contract* c : g_contracts)
    if (c->get_argument() == arg)
    {
      CONSOLE("Contract already exists (skip)\n");
      return newcntrs;
    }

  // register the new argument for each existing non-parameterized contract
  for (Contract* c : g_contracts)
    if (c->get_argument().empty())
    {
      Contract* cnew = new Contract(*c, arg);
#if VERBOSITY_LEVEL >= 10
      CONSOLE("Creating new contract: " + cnew->toString() + "\n");
#endif
      newcntrs.push_back(cnew);
    }

  g_contracts.insert(g_contracts.end(), newcntrs.begin(), newcntrs.end());
  CONSOLE("Added " + decstr(newcntrs.size()) + " new contract(s).\n");
