...
class IntervalMap
{
  ...
  public: // Inline generated methods
     ...
     *   the key belongs or Map::end if the key does not belong to any interval.
     */
    typename Map::iterator find(const KEY& key)
    {
      // Try to find the interval to which the key belongs
      typename Map::iterator it = m_map.lower_bound(key);
     ...
     */
    std::pair< typename Map::iterator, bool > insert(const KEY& min,
      const KEY& max, const VALUE& value)
    {
